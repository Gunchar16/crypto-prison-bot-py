...
status = [
    'Counting Ethereum', 'Collecting NFTs', 'Dropping $OAP',
    'Solving Sneaky Peakys', 'Covering Tracks For Gramps', 'Hiding From M'
]
...
async def change_status():
    await client.change_presence(activity=discord.Game(
        np.random.choice(status, p=[0.2, 0.2, 0.32, 0.2, 0.06, 0.02])))
...
async def get_random(ctx: SlashContext, number: int):
    ...
    embed = discord.Embed(
        color=0xd17000,
        title='The Winner' if number == 1 else 'The List of Winners',
        description='The Winner!' if number == 1 else 'The List of Winners!')
    random_user = []
    for x in range(number):
        if (len(random_user) == 0):
            ...
            embed.add_field(
                name=f'Winner Number {x + 1}:' if number != 1 else 'Winner:',
                value=choose)
        else:
            choose = random.choice(client.get_guild(guild_id).members)
